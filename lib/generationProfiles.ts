...
export interface RegionProfile {
  ...
  readonly generationDefaults: Readonly<Partial<Record<GenerationMode, GenerationDefaults>>>;
}
...
const REGION_PROFILES: Readonly<Record<AnatomicalRegion, RegionProfile>> = {
  face: {
    iconPath:
...
export function getPreferredModeForRegion(region: AnatomicalRegion): GenerationMode {
  return REGION_ALLOWED_MODES[region][0];
}

export function getDefaultScanTypeForRegion(region: AnatomicalRegion): ScanType {
  return REGION_PREFERRED_SCAN_TYPE[region];
}
...
export function getGenerationDefaults(
  ...
  const safeMode = isModeAllowedForRegion(mode, region) ? mode : getPreferredModeForRegion(region);
  const safeDefaults = profile.generationDefaults[safeMode];

  if (safeDefaults) {
    return safeDefaults;
  }

  return profile.generationDefaults[getPreferredModeForRegion(region)]!;
}
